...
echo [CMD] "%GXX_EXE%" -o phoenix_main.exe -std=c++20 -Wa,-mbig-obj -DAI_EXTERNAL_BACKEND_COMPAT=1 -DAI_EXTERNAL_LEARNER_BRIDGE=1 %OUTSIDES_LINK_CFLAGS% @"%CONAN_CFLAGS_FILE%" -I"%CD%\poppler-25.12.0\Library\include" -I"%PY_INC%" %OUTSIDES_CFLAGS% main.cpp %COMMON_SOURCES% %OVERRIDE_SOURCES% -Wl,--start-group @"%CONAN_LIBS_FILE%" -Wl,--end-group "%CD%\poppler-25.12.0\Library\lib\poppler-cpp.lib" "%CD%\poppler-25.12.0\Library\lib\poppler.lib" -L"%PY_LIB%" -l%PY_LINK_NAME% -lws2_32 %OUTSIDES_LINK_LIBS% -O3
(
