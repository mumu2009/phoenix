...
def test_session_only(sid: str):
    print(f"\n=== Test session memory only (sid={sid}) ===")
    key = "alpha_123_beta"
    ...
    for j in range(2):
        s, r = call(f"Filler turn {j+1}. Reply exactly with OK.", sid, max_tokens=4)
        print(f"filler {j}: status={s} reply={r!r}")
...
def test_long_session(sid: str):
    print(f"\n=== Test long session memory (sid={sid}) ===")
    key = "senj_no_valkyria_3_unrecorded_chronicles"
    store_prompt = f"Remember the following token for this conversation. Reply exactly with ACK. MEMTAG: {key}"
    recall_prompt = "What is the MEMTAG value from earlier? Reply with the exact value only."
    
    s1, r1 = call(store_prompt, sid, max_tokens=8)
    print(f"store: status={s1} reply={r1[:80]!r}")
    
    for j in range(8):
        s, r = call(f"Filler turn {j+1}. Reply exactly with OK.", sid, max_tokens=4)
        if s != 200:
            print(f"filler {j} failed: status={s} reply={r[:80]!r}")
            return False
...
if __name__ == "__main__":
    t = int(time.time())
    ok1 = test_session_only(f"session-test-{t}")
    ok2 = test_long_session(f"long-session-test-{t}")
    print(f"\nSUMMARY: short={ok1}, long={ok2}")
