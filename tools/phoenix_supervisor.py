...
def default_services(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "host": str(llama.get("host") or "127.0.0.1"),
            "maxRssMb": 0,
...
def observe(spec: dict[str, Any], pid_dir: Path) -> dict[str, Any]:
    ...
    alive_port = port_listening(spec["host"], int(spec["port"]))
    alive = alive_pid or alive_port
    rss = rss_mb(pid) if alive_pid else 0
    max_rss = int(spec.get("maxRssMb") or 0)
    # RSS fuse never applies to llama (leave it running).
    over_mem = bool(role != "llama" and max_rss > 0 and rss >= max_rss)
    return {
        "role": role,
        "pid": pid,
        "port": int(spec["port"]),
...
def start_service(role: str, cmd: str, cwd: Path, extra_env: dict[str, str]) -> int:
    if not cmd.strip():
...
def plan_and_act(
    ...
    for svc in observed:
        ...
        if svc.get("overMem") and role != "llama":
        ...
        if dry_run:
            continue
        cmd = os.environ.get(str(spec["cmd_env"]), "")
        pid = start_service(role, cmd, cwd, {})
        if pid:
...
def main() -> int:
    ...
    while True:
        ...
        decisions = plan_and_act(observed, specs, repo, args.dry_run)
        payload = {
            "version": 1,
            "product": PRODUCT,
            "writtenAt": utc_now(),
