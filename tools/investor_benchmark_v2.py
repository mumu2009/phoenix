...
def with_retries(fn, retries: int = 2, sleep_s: float = 0.7):
    last = None
...
def semantic_match(pred: str, ref: str, threshold: float = 0.48) -> tuple[bool, float]:
...
def build_instruction_prompt(ex: Gpt4AllExample) -> str:
    ...
    if ex.input_text:
        return (
            "Follow the instruction and produce a helpful answer in English.\n"
...
def warmup_ollama(model: str, ollama_url: str, timeout_s: float, context_window: int) -> ProviderCall:
    ...
    return call_ollama_messages([{"role": "user", "content": prompt}], model, ollama_url, timeout_s, context_window, max_tokens=8)
...
def benchmark_instruction(
    ...
    for idx, ex in enumerate(examples, start=1):
        ...
        ollama = call_ollama_messages([{"role": "user", "content": prompt}], model, ollama_url, timeout_s, context_window, max_tokens=64)
        phoenix = call_phoenix_transformer_chat(prompt, phoenix_token, phoenix_transformer_url, f"{run_prefix}-inst-{idx}", timeout_s, max_tokens=64)
        _, ollama_score = semantic_match(ollama.reply, ex.reference)
        _, phoenix_score = semantic_match(phoenix.reply, ex.reference)
        results.append(InstructionResult(example_id=ex.example_id, ollama=ollama, phoenix=phoenix, ollama_score=ollama_score, phoenix_score=phoenix_score))
        if idx == 1 or idx == total or idx % 10 == 0:
            safe_log(f"[instruction] {idx}/{total} ollama_status={ollama.status} phoenix_status={phoenix.status}")
    return results
...
def benchmark_window_memory(
    ...
    for idx, ex in enumerate(examples, start=1):
        ...
        phoenix_store = call_phoenix_transformer_chat(remember_prompt, phoenix_token, phoenix_transformer_url, sid, timeout_s, max_tokens=16)
        phoenix_recall = call_phoenix_transformer_chat(recall_prompt, phoenix_token, phoenix_transformer_url, sid, timeout_s, max_tokens=32)
        ...
        results.append(WindowResult(example_id=ex.example_id, ollama=ollama_recall, phoenix=phoenix_recall, ollama_score=ollama_score, phoenix_score=phoenix_score))
        if idx == 1 or idx == total or idx % 10 == 0:
            safe_log(f"[window] {idx}/{total} ollama_status={ollama_recall.status} phoenix_status={phoenix_recall.status}")
    return results
...
def build_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Executive Summary",
        "",
        f"- Instruction semantic accuracy: Ollama {inst['ollama']['accuracy']:.3f}% ({inst['ollama']['success']}/{inst['ollama']['total']}) vs 079 {inst['phoenix']['accuracy']:.3f}% ({inst['phoenix']['success']}/{inst['phoenix']['total']}).",
        f"- Window-memory semantic accuracy: Ollama {win['ollama']['accuracy']:.3f}% ({win['ollama']['success']}/{win['ollama']['total']}) vs 079 {win['phoenix']['accuracy']:.3f}% ({win['phoenix']['success']}/{win['phoenix']['total']}).",
        f"- Chat latency median (instruction): Ollama {inst['ollamaLatency']['median']:.2f} ms vs 079 {inst['phoenixLatency']['median']:.2f} ms.",
        f"- Chat latency median (window): Ollama {win['ollamaLatency']['median']:.2f} ms vs 079 {win['phoenixLatency']['median']:.2f} ms.",
        "",
        "## Gateway Health Check",
        "",
        f"- Instruction route HTTP errors: Ollama {inst['ollamaHttpErrors']} | 079 {inst['phoenixHttpErrors']}",
        f"- Window route HTTP errors: Ollama {win['ollamaHttpErrors']} | 079 {win['phoenixHttpErrors']}",
        "",
        "## Notes",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run GPT4All-sourced benchmark for 079 vs direct Ollama")
