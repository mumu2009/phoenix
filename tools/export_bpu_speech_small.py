#!/usr/bin/env python3
"""Export a small, BPU-friendly 1D speech autoencoder to ONNX.

Architecture is chosen so every op is supported by Horizon RDK X5 BPU
(march=bayes-e) while keeping the total parameter count low enough for
black-box / evolutionary search (~70k parameters).

Input:  [N, 1, 1, 16000]  (16 kHz mono waveform)
Output: [N, 1, 1, 16000]  (reconstructed waveform)
Bottleneck concept: [N, 1, 1, 32]
"""
...
class SmallSpeechEncoder(nn.Module):
    """1x1x1x16000 -> 1x1x1x32 concept."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(-1, 64)        # [N,64]  (dynamic batch via -1)
        x = self.fc(x)            # [N,32]
        # BPU-friendly 4D concept tensor.
        return x.view(-1, self.concept, 1, 1)


class SmallSpeechDecoder(nn.Module):
    """1x1x1x32 concept -> 1x1x1x16000 waveform."""
    def __init__(self, concept: int = CONCEPT):
        super().__init__()
        self.concept = concept
        # 32 -> 8*1*250 = 2000, then 3 ConvTranspose of stride 4 to reach 16000.
...
class SmallSpeechAutoencoder(nn.Module):
    def __init__(self, concept: int = CONCEPT):
        super().__init__()
        self.encoder = SmallSpeechEncoder(concept)
        self.decoder = SmallSpeechDecoder(concept)
...
def main() -> int:
    ...
    model = SmallSpeechAutoencoder(args.concept)
    ...
    write_calibration(out_dir, "decoder", (1, args.concept), args.calibration_count)
