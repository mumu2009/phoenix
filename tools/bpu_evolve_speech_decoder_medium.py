#!/usr/bin/env python3
"""Black-box (1+lambda) evolution of the medium speech autoencoder decoder.
...
Kali (192.168.1.100) has no route to the X5 (192.168.1.107); the Windows host
...
2. On Kali: start from the exported ``init5`` decoder ONNX, mutate the
...
Usage (run from a Windows host inside the Phoenix repo):
    ...
    python tools\\bpu_evolve_speech_decoder_medium.py \
        --kali-host 192.168.1.100 --kali-user kali --kali-pass kali \
        --x5-host 192.168.1.107 --x5-user sunrise --x5-pass sunrise \
...
KALI_WORK = PurePosixPath("/media/sf_phoenix/speech_evolve/evolve_medium")
KALI_INIT = PurePosixPath("/media/sf_phoenix/speech_evolve/init5")
KALI_DATASET = PurePosixPath("/home/kali/phoenix/datasets/musan_16k")

X5_WORK = PurePosixPath("/home/sunrise/phoenix/evolve_medium")
...
def compile_candidate(args: argparse.Namespace) -> None:
    ...
    wrapper = "/workspace/speech/compile_bpu_speech_decoder_medium.sh"
...
class KaliPipeline:
    ...
    def setup(self) -> None:
        ...
        self.host.upload_text(COMPILE_WRAPPER, "/media/sf_phoenix/speech_evolve/compile_bpu_speech_decoder_medium.sh", 0o755)
...
def main() -> int:
    ...
    for gen in range(args.generations):
        ...
        # .bin is reused from the init5 compilation if it exists; otherwise we
        # compile the un-mutated decoder as candidate_0 later.
        base_bin = init_dir / "speech_bin" / "model_decoder.bin"
