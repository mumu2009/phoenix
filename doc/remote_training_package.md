...
python tools\train_pilot.py `
    ...
    --out-dir <checkpoints-dir>\audio_pilot `
    --epochs 3 --batch-size 8 --device auto
```

This creates `<checkpoints-dir>\audio_pilot\best.pt` and `model_*.onnx`.
...
python tools\train_audio.py `
    --data-dir <datasets-dir>\musan_16k `
    --out-dir <checkpoints-dir>\audio_small `
    --scale small `
    --epochs 50 `
    --batch-size 32 `
    --lr 1e-3 `
...
python tools\export_multimodal.py `
    --modality speech `
    --checkpoint <checkpoints-dir>\audio_small\best.pt `
    --out-dir <checkpoints-dir>\audio_small\onnx
```
...
python tools\train_video.py `
    ...
    --out-dir <checkpoints-dir>\video_small `
    --resnet resnet18 `
    --concept 128 `
    --decoder-pt none `
    --decoder-width 1.25 `
...
python tools\export_multimodal.py `
    --modality image `
    --checkpoint <checkpoints-dir>\video_small\best.pt `
    --out-dir <checkpoints-dir>\video_small\onnx
```
...
Start-Process -FilePath python -ArgumentList @(
    "tools\train_audio.py",
    "--data-dir", "<datasets-dir>\musan_16k",
    "--out-dir", "<checkpoints-dir>\audio_medium",
    "--scale", "medium",
    "--epochs", "100",
    "--batch-size", "16",
    "--amp",
