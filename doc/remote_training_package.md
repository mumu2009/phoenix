...
- **Speech**: MUSAN 16 kHz mono WAV files.  On the remote box point `--data-dir`
  at the folder containing `*.wav`.  Example: `<datasets-dir>\musan_16k`.
...
python tools\train_jpea_v2_pilot.py `
    --data-dir <datasets-dir>\musan_16k `
    --out-dir <checkpoints-dir>\speech_pilot `
    --epochs 3 --batch-size 8 --device auto
```

This creates `<checkpoints-dir>\speech_pilot\best.pt` and `model_*.onnx`.
...
python tools\train_jpea_v2_speech.py `
    --data-dir <datasets-dir>\musan_16k `
    --out-dir <checkpoints-dir>\jpea_v2_speech_small `
    --scale small `
    --epochs 50 `
    --batch-size 32 `
    --lr 1e-3 `
...
python tools\export_jpea_v2_multimodal.py `
    --modality speech `
    --checkpoint <checkpoints-dir>\jpea_v2_speech_small\best.pt `
    --out-dir <checkpoints-dir>\jpea_v2_speech_small\onnx
```
...
python tools\train_jpea_v2_vision.py `
    --data-dir <datasets-dir>\images `
    --out-dir <checkpoints-dir>\jpea_v2_vision_small `
    --resnet resnet18 `
    --concept 128 `
    --decoder-pt none `
    --decoder-width 1.25 `
...
python tools\export_jpea_v2_multimodal.py `
    --modality image `
    --checkpoint <checkpoints-dir>\jpea_v2_vision_small\best.pt `
    --out-dir <checkpoints-dir>\jpea_v2_vision_small\onnx
```
...
The remote GPU is a mid-range CUDA card with ~6 GB.  Use these rules of thumb:
...
```powershell
$log = "<checkpoints-dir>\speech_medium.log"
Start-Process -FilePath python -ArgumentList @(
    "tools\train_jpea_v2_speech.py",
    "--data-dir", "<datasets-dir>\musan_16k",
    "--out-dir", "<checkpoints-dir>\jpea_v2_speech_medium",
    "--scale", "medium",
    "--epochs", "100",
    "--batch-size", "16",
    "--amp",
