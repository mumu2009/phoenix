...
面向 192.168.1.107（RDK X5, aarch64）：宿主机（192.168.1.104）只做**一件事**：HTTP 提供文件（:8090）。RDK 自己在设备上
...
```powershell
# 一次性拉取 RDK sysroot
bash tools/rdk_sysroot_fetch.sh sunrise@192.168.1.107
...
# 局域网 / 物理网口
curl -s http://192.168.1.107:5080/api/system/status | head -c 200
curl -s -o /dev/null -w "%{http_code}" http://192.168.1.107:5081/
```
...
for f in rdk_netboot_fetch.py rdk_netboot_build_and_run.sh rdk_llama_setup.sh \
  ...
  curl -fsSL -m 60 "http://192.168.1.104:8090/tools/$f" -o "$ROOT/tools/$f"
done
chmod +x "$ROOT"/tools/*.sh
python3 "$ROOT/tools/rdk_netboot_fetch.py" \
  "http://192.168.1.104:8090/build/rdk_netboot_manifest.txt" "$ROOT" \
