...
| Remote / local vision / speech models | `video_model.cpp`, `audio_model.cpp` | `MixedModalIOTest.*` (runtime) | Local ONNX / BPU / server-client / unavailable model selection, additive residual `.onnx`/`.bin` resolution | done |
...
- The factories create `VideoLocalOnnxModel` / `AudioLocalOnnxModel`
  for the x86_64 `cpu`/`gpu` local backend, `VideoHbdnnModel` /
  `AudioHbdnnModel` for the RDK X5 `bpu` backend, and
  `Video/Audio*UnavailableModel` when the required `.onnx` or `.bin` is missing.
- `VideoServerClientModel` and `AudioServerClientModel` reject
...
- `VideoHbdnnModel` now falls back to `runtime_store/models/ijepa/` when
...
- `AudioFallbackModel::adapt` now runs a real self-supervised
  JEPA step: random context/target window splits, a learned linear
  predictor, and SGD on the MSE loss.
- `AudioRemoteModel::encodeTarget` and `contrastiveAdapt` are no
  ...
  remote embedding.
- `VideoHbdnnModel` and `VideoRemoteModel` `encodeTarget` and
