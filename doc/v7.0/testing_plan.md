...
- `JpeaV2ImageHbdnnModel` now falls back to `runtime_store/models/ijepa/` when
  the `JPEA_IMAGE_HORIZON_MODEL`/`JPEA_IMAGE_HORIZON_DECODER` environment
...
- `JpeaV2SpeechFallbackModel::adapt` now runs a real self-supervised
  JEPA step: random context/target window splits, a learned linear
  predictor, and SGD on the MSE loss.
- `JpeaV2SpeechRemoteModel::encodeTarget` and `contrastiveAdapt` are no
  ...
  remote embedding.
- `JpeaV2ImageHbdnnModel` and `JpeaV2ImageRemoteModel` `encodeTarget` and
