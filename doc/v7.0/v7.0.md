...
## 12. 自主智能时代（v7.0 后期合入，全部可选/默认关闭）

本阶段把 Phoenix 从"反应式对话系统"推进为**长期自主运行的自进化智能体**，并配套系统级安全装置：

1. **自主推理**：主动推理/MPC（EFE 三项、滚动时域、前向 latent 模型）——`active_inference.md`、`algorithm.md` §18；
2. **自主进化**：TD(0) 价值学习 + 在线前向模型 + 情景记忆巩固 + VDBE 自适应探索；潜意识剖面（PAD/稳态）提供可遗传参数——`subconscious.md`；
3. **任务层**：Meeseeks 盒（疼痛 p(t)=g·t，总疼痛 gT²/2 定理；复制是实例的自由能力、有界无选择无接班）——`mission_layer.md`；
4. **插件生态**：cli-json 桥（任意 CLI 软件→插件）、MCP 兼容（主流插件市场）、精确数学与线上搜索插件——`plugins.md`；
5. **长期化**：自主心跳（无外部消息持续 plan/act/observe/learn）+ 全状态持久化（重启进化不丢）+ 人类插话（中途提示/改写目标）——`autonomy_loop.md`；
6. **系统级安全**：多实例急停（实例生命周期登记 → 一键全杀 → 自我关闭）——`safety.md`；单实例层为既有 memebarrier。

验证：gtest 全量 **4148 测试 0 失败**（含 649 部署配置 × 3 的参数化矩阵）；协议见 `testing_methodology.md` §1–§12。