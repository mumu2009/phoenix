...
1. **输入模态**：文本、图像、音频、视频、传感器统一进入 `aheadModule`；音频/视频的编码器由 `aheadModule` 内部实现（对应 `JepaV2SpeechWorldModel`/`JepaV2ImageWorldModel`），文本编码器可直接复用 `llama3.1 8b` 自带的 embedding/encoder 部分，不必单独训练 —— 呼应 §I.1 "以目前 llama3.1:8b 的维度数为准，不希望大规模调整"。
...
- `dec`：unit query -> 输出模态。文本场景复用 llama.cpp 的 LM head 产生 token 并 detokenize；音频/视频场景由于 `.gguf` / raw blob 本身不包含对应的解码权重、且这些权重不入库，`dec` 改为 Phoenix 运行期 C++ 代码实现（`jepa_v2_image_world_model.cpp`/`jepa_v2_speech_world_model.cpp` 的 `decode()`）。
