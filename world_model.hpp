...
struct ConsciousComputeOptions {
    std::size_t maxStages{6};
    std::size_t maxHumanPrompts{4};
    std::size_t maxMachineSteps{4};
    std::size_t maxChars{160};
    bool requireVoluntaryParticipation{true};
};

struct CollectiveComputeOptions {
    std::size_t participantCount{48};
    std::size_t shardCount{8};
    std::size_t redundancyFactor{3};
    std::size_t maxMnemonicWords{8};
    std::size_t maxCandidatesPerShard{5};
    std::size_t maxChars{160};
    double acceptableRelativeError{0.18};
    bool allowMnemonicEncoding{true};
    bool requireConsensus{true};
    bool allowParticipantRoutingStorage{true};
    bool ignoreMatchingForComputeNodes{true};
};
...
inline json buildConsciousComputePlan(const json &sessionState, ConsciousComputeOptions options = {}) {
    BrainProfileOptions brainOptions;
    brainOptions.kind = BrainProfileKind::Structural;
    brainOptions.maxChars = options.maxChars;
    const auto structural = isStructuralBrainState(sessionState) ? sessionState : buildStructuralBrainCore(sessionState, brainOptions);
    auto thoughtModel = buildHumanThoughtModel(structural, brainOptions);

    const auto focus = collectJsonStrings(structural.value("attentionFocus", json::array()), 3);
    const auto goals = collectJsonStrings(structural.value("activeGoals", json::array()), 3);
    const std::string sceneSummary = truncateText(structural.value("sceneSummary", std::string()), options.maxChars);

    json machinePrecompute = json::array({truncateText("Fuse current visual, auditory, and episodic evidence into a compact world state.", options.maxChars),
                                          truncateText("Generate at most three counterfactual branches from the structural brain pathways.", options.maxChars),
                                          truncateText("Rank attention targets before asking the human for conscious judgment.", options.maxChars)});

    json humanStages = json::array({json{{"stage", "sensory_focus"},
                                        {"prompt", truncateText("请只保留你看到/听到的两个最关键线索，并说出它们为何重要。", options.maxChars)}},
                                    json{{"stage", "episodic_analogy"},
                                        {"prompt", truncateText("请回忆最像当前情况的一段经验或直觉原型，用一句话概括。", options.maxChars)}},
                                    json{{"stage", "branch_pruning"},
                                        {"prompt", truncateText("只保留两个最可能的后续分支，哪一个更合理，为什么？", options.maxChars)}},
                                    json{{"stage", "action_commitment"},
                                        {"prompt", truncateText("如果现在必须行动，第一步应该做什么？", options.maxChars)}}});

    json machinePostprocess = json::array({truncateText("Encode the human answer as a high-level prior instead of replaying exhaustive search.", options.maxChars),
                                           truncateText("Verify the chosen branch against multimodal consistency and existing memory.", options.maxChars),
                                           truncateText("Write the accepted judgment back into the world model and training stream.", options.maxChars)});

    if (machinePrecompute.size() > options.maxMachineSteps) {
        machinePrecompute.erase(machinePrecompute.begin() + static_cast<json::difference_type>(options.maxMachineSteps), machinePrecompute.end());
    }
    if (humanStages.size() > options.maxHumanPrompts) {
        humanStages.erase(humanStages.begin() + static_cast<json::difference_type>(options.maxHumanPrompts), humanStages.end());
    }
    if (machinePostprocess.size() > options.maxMachineSteps) {
        machinePostprocess.erase(machinePostprocess.begin() + static_cast<json::difference_type>(options.maxMachineSteps), machinePostprocess.end());
    }

    if (thoughtModel.contains("phases") && thoughtModel["phases"].is_array() && thoughtModel["phases"].size() > options.maxStages) {
        thoughtModel["phases"].erase(thoughtModel["phases"].begin() + static_cast<json::difference_type>(options.maxStages), thoughtModel["phases"].end());
    }

    const double branchReduction = std::max(0.25, std::min(0.80, 0.32 + 0.10 * static_cast<double>(focus.size())));
    const double tokenReduction = std::max(0.20, std::min(0.70, 0.26 + 0.08 * static_cast<double>(goals.size())));

    return json{{"ok", true},
                {"mode", "human-in-the-loop-conscious-compute"},
                {"requiresVoluntaryParticipation", options.requireVoluntaryParticipation},
                {"targetScene", sceneSummary},
                {"machinePrecompute", machinePrecompute},
                {"humanStages", humanStages},
                {"machinePostprocess", machinePostprocess},
                {"estimatedSavings", json{{"branchReduction", branchReduction}, {"tokenReduction", tokenReduction}}},
                {"protocolAssumptions", json::array({"Human participation is explicit and voluntary.", "Human responses are used as compact priors, not as hidden-state extraction.", "The machine still verifies multimodal consistency after human input."})},
                {"thoughtModel", thoughtModel}};
}

inline json buildCollectiveConsciousComputePlan(const json &sessionState, const json &computeTask, CollectiveComputeOptions options = {}) {
    BrainProfileOptions brainOptions;
    brainOptions.kind = BrainProfileKind::Structural;
    brainOptions.maxChars = options.maxChars;
    const auto structural = isStructuralBrainState(sessionState) ? sessionState : buildStructuralBrainCore(sessionState, brainOptions);
    const auto thoughtModel = buildHumanThoughtModel(structural, brainOptions);

    const std::string operation = trimCopy(computeTask.value("operation", std::string("semantic_approximation")));
    const std::string sceneSummary = truncateText(structural.value("sceneSummary", std::string()), options.maxChars);
    const auto focus = collectJsonStrings(structural.value("attentionFocus", json::array()), 4);
    const auto goals = collectJsonStrings(structural.value("activeGoals", json::array()), 4);
    const auto heuristics = collectJsonStrings(thoughtModel.value("thoughtHeuristics", json::array()), 4);

    std::vector<std::string> routingInputs = collectJsonStrings(computeTask.value("semanticInputs", json::array()), 6);
    {
        std::unordered_set<std::string> seen(routingInputs.begin(), routingInputs.end());
        if (computeTask.contains("inputA") && computeTask["inputA"].is_string()) {
            appendUniqueLimited(routingInputs, seen, computeTask["inputA"].get<std::string>(), 6, options.maxChars);
        }
        if (computeTask.contains("inputB") && computeTask["inputB"].is_string()) {
            appendUniqueLimited(routingInputs, seen, computeTask["inputB"].get<std::string>(), 6, options.maxChars);
        }
    }
    if (routingInputs.empty()) {
        std::unordered_set<std::string> seen;
        for (const auto &item : focus) {
            appendUniqueLimited(routingInputs, seen, item, 6, options.maxChars);
        }
        for (const auto &item : heuristics) {
            appendUniqueLimited(routingInputs, seen, item, 6, options.maxChars);
        }
    }

    std::vector<std::string> outputHints = collectJsonStrings(computeTask.value("outputHints", json::array()), 4);
    {
        std::unordered_set<std::string> seen(outputHints.begin(), outputHints.end());
        if (computeTask.contains("desiredOutput") && computeTask["desiredOutput"].is_string()) {
            appendUniqueLimited(outputHints, seen, computeTask["desiredOutput"].get<std::string>(), 4, options.maxChars);
        }
    }
    if (outputHints.empty()) {
        std::unordered_set<std::string> seen;
        for (const auto &item : goals) {
            appendUniqueLimited(outputHints, seen, item, 4, options.maxChars);
        }
    }

    auto readShape = [](const json &shape, std::size_t index) -> std::size_t {
        if (!shape.is_array() || shape.size() <= index || !shape[index].is_number_integer()) {
            return 0;
        }
        return static_cast<std::size_t>(std::max<int64_t>(0, shape[index].get<int64_t>()));
    };

    const std::size_t leftRows = readShape(computeTask.value("leftShape", json::array()), 0);
    const std::size_t innerDim = readShape(computeTask.value("leftShape", json::array()), 1);
    const std::size_t rightCols = readShape(computeTask.value("rightShape", json::array()), 1);
    const std::size_t blockRows = std::max<std::size_t>(1, computeTask.value("blockRows", 16));
    const std::size_t blockCols = std::max<std::size_t>(1, computeTask.value("blockCols", 16));

    std::size_t inferredShardCount = options.shardCount;
    if (operation == "matrix_multiply" && leftRows > 0 && rightCols > 0) {
        const std::size_t rowBlocks = (leftRows + blockRows - 1) / blockRows;
        const std::size_t colBlocks = (rightCols + blockCols - 1) / blockCols;
        inferredShardCount = std::max<std::size_t>(1, rowBlocks * colBlocks);
    }
    const std::size_t shardCount = std::max<std::size_t>(1, std::min(options.shardCount, inferredShardCount));

    std::vector<std::string> semanticAnchors;
    std::unordered_set<std::string> anchorSeen;
    for (const auto &item : focus) {
        appendUniqueLimited(semanticAnchors, anchorSeen, item, options.maxCandidatesPerShard, options.maxChars);
    }
    for (const auto &item : goals) {
        appendUniqueLimited(semanticAnchors, anchorSeen, item, options.maxCandidatesPerShard, options.maxChars);
    }
    ...
    static const std::vector<std::string> kDefaultRelationWords = {"align", "merge", "route", "amplify", "suppress", "stabilize", "contrast", "transfer"};
    for (const auto &item : kDefaultRelationWords) {
        appendUniqueLimited(semanticAnchors, anchorSeen, item, options.maxCandidatesPerShard, options.maxChars);
    }

    json humanShards = json::array();
    json mnemonicHints = json::array();
    json routeQueries = json::array();
    json participantDirectory = json::array();
    const std::size_t participantsNeeded = std::max<std::size_t>(1, shardCount * std::max<std::size_t>(1, options.redundancyFactor));
    const std::size_t availableParticipants = std::max<std::size_t>(1, options.participantCount);
    const std::string matchingPriority = options.ignoreMatchingForComputeNodes ? "routing-storage-only" : "compute-and-routing";
    static const std::vector<std::string> kParticipantRoles = {
        "semantic-composer",
        "contrast-judge",
        "stability-judge",
        "consensus-checker",
    ...
    if (routingInputs.empty()) {
        routingInputs.push_back("align");
        routingInputs.push_back("route");
    }
    if (outputHints.empty()) {
        outputHints.push_back("stabilize");
    }

    for (std::size_t participantIndex = 0; participantIndex < availableParticipants; ++participantIndex) {
        const std::string inputA = routingInputs[participantIndex % routingInputs.size()];
        const std::string inputB = routingInputs[(participantIndex + 1) % routingInputs.size()];
        const std::string outputFamily = outputHints[participantIndex % outputHints.size()];
        const std::string role = kParticipantRoles[participantIndex % kParticipantRoles.size()];
        const double reliability = std::max(0.55, std::min(0.95, 0.62 + 0.03 * static_cast<double>(participantIndex % 5)));
        json participant{{"participantSlot", participantIndex + 1},
                         {"role", role},
                         {"voluntary", true},
                         {"revocable", true},
                         {"computeEligible", true},
                         {"routingEligible", options.allowParticipantRoutingStorage}};
        if (options.allowParticipantRoutingStorage) {
            participant["routingSignature"] = json{{"inputPair", json::array({inputA, inputB})},
                                                    {"outputFamily", outputFamily},
                                                    {"reliability", reliability},
                                                    {"bestFor", operation}};
        } else {
            participant["genericCapability"] = json{{"matchingImportance", options.ignoreMatchingForComputeNodes ? "low" : "high"},
                                                      {"bestFor", operation},
                                                      {"reliability", reliability}};
        }
        participantDirectory.push_back(participant);
    }

    json genericComputePool{{"matchingImportance", options.ignoreMatchingForComputeNodes ? "low" : "high"},
                            {"assignmentStrategy", options.ignoreMatchingForComputeNodes ? "capacity-first" : "signature-guided"},
                            {"eligibleParticipants", availableParticipants},
                            {"routingStorageEnabled", options.allowParticipantRoutingStorage}};

    for (std::size_t shardIndex = 0; shardIndex < shardCount; ++shardIndex) {
        json candidates = json::array();
        for (std::size_t candidateIndex = 0; candidateIndex < std::min(options.maxCandidatesPerShard, semanticAnchors.size()); ++candidateIndex) {
            const std::size_t offset = (shardIndex + candidateIndex) % semanticAnchors.size();
            candidates.push_back(semanticAnchors[offset]);
        }

        const std::string inputA = routingInputs[shardIndex % routingInputs.size()];
        const std::string inputB = routingInputs[(shardIndex + 1) % routingInputs.size()];
        const std::string desiredOutput = outputHints[shardIndex % outputHints.size()];

        std::ostringstream prompt;
        if (operation == "matrix_multiply") {
            prompt << "针对矩阵分片 " << (shardIndex + 1)
                   << "，给定输入词 " << inputA << ", " << inputB
                   << "，在允许误差范围内选择最接近 " << desiredOutput
                   << " 的语义关系词，用于近似该块的乘法贡献。";
        } else {
            prompt << "针对计算分片 " << (shardIndex + 1)
                   << "，给定输入词 " << inputA << ", " << inputB
                   << "，选择最贴近 " << desiredOutput << " 的语义关系词。";
        }

        json assignedParticipants = json::array();
        json matchedParticipants = json::array();
        for (std::size_t replica = 0; replica < std::max<std::size_t>(1, options.redundancyFactor); ++replica) {
            const std::size_t participantSlot = options.participantCount == 0 ? 0 : ((shardIndex * options.redundancyFactor + replica) % options.participantCount);
            assignedParticipants.push_back(json{{"participantSlot", participantSlot + 1},
                                               {"role", replica == 0 ? "primary" : "redundant"},
                                               {"assignmentMode", options.ignoreMatchingForComputeNodes ? "generic-compute" : "signature-routed"}});
            if (options.allowParticipantRoutingStorage) {
                matchedParticipants.push_back(json{{"participantSlot", participantSlot + 1},
                                                  {"matchReason", options.ignoreMatchingForComputeNodes ? "routing storage only" : "input pair and output family routing"}});
            }
        }

        humanShards.push_back(json{{"shardId", shardIndex + 1},
                                   {"prompt", truncateText(prompt.str(), options.maxChars)},
                                   {"operation", operation},
                                   {"inputPair", json::array({inputA, inputB})},
                                   {"desiredOutputFamily", desiredOutput},
                                   {"candidates", candidates},
                                   {"assignedParticipants", assignedParticipants},
                                   {"approximationWindow", json{{"acceptableRelativeError", options.acceptableRelativeError},
                                                                 {"redundancyFactor", options.redundancyFactor}}}});

        if (options.allowParticipantRoutingStorage) {
            routeQueries.push_back(json{{"shardId", shardIndex + 1},
                                        {"inputPair", json::array({inputA, inputB})},
                                        {"desiredOutputFamily", desiredOutput},
                                        {"matchedParticipants", matchedParticipants}});
        }

        if (options.allowMnemonicEncoding) {
            std::vector<std::string> words;
            for (std::size_t index = 0; index < std::min(options.maxMnemonicWords, semanticAnchors.size()); ++index) {
                words.push_back(semanticAnchors[(shardIndex + index) % semanticAnchors.size()]);
            }
            mnemonicHints.push_back(json{{"shardId", shardIndex + 1},
                                         {"mnemonic", truncateText(joinStrings(words, " "), options.maxChars)},
                                         {"retention", "ephemeral"},
                                         {"expiresAfter", "single task or short session"}});
        }
    }

    const double baselineOps = (operation == "matrix_multiply" && leftRows > 0 && innerDim > 0 && rightCols > 0)
                                   ? static_cast<double>(leftRows) * static_cast<double>(innerDim) * static_cast<double>(rightCols)
                                   : static_cast<double>(shardCount) * 1024.0;
    const double reductionBase = std::min(0.88, 0.35 + 0.02 * static_cast<double>(std::min<std::size_t>(options.participantCount, 64)) + 0.04 * static_cast<double>(std::min<std::size_t>(options.redundancyFactor, 4)));
    const double guidedOps = baselineOps * std::max(0.12, 1.0 - reductionBase);

    return json{{"ok", true},
                {"mode", "collective-semantic-compute"},
                {"operation", operation},
                {"targetScene", sceneSummary},
                                {"routingDirectory", json{{"storageMode", options.allowParticipantRoutingStorage ? "participant-signature-lookup" : "disabled"},
                                                                                     {"stores", options.allowParticipantRoutingStorage ? "volunteer routing signatures instead of human memory content" : "routing signatures are disabled; volunteers stay compute-only"},
                                                                                     {"lookupRule", options.allowParticipantRoutingStorage ? "match input pair and desired output family to volunteer signatures" : "skip signature lookup and allocate any voluntary compute node"},
                                                                                     {"matchingPriority", matchingPriority},
                                                                                     {"entries", participantDirectory},
                                                                                     {"routeQueries", routeQueries}}},
                {"collectiveMesh", json{{"participantCount", options.participantCount},
                                         {"minimumParticipants", participantsNeeded},
                                                                                 {"capacityStatus", options.participantCount >= participantsNeeded ? "sufficient" : "underprovisioned"},
                                         {"shardCount", shardCount},
                                         {"redundancyFactor", options.redundancyFactor},
                                                                                 {"genericComputePool", genericComputePool},
                                         {"humanShards", humanShards},
                                         {"mnemonicHints", mnemonicHints}}},
                {"aggregation", json{{"strategy", options.requireConsensus ? "redundant semantic voting + machine verification" : "single vote + machine verification"},
                                      {"consensusThreshold", options.requireConsensus ? 0.67 : 0.0},
                                      {"acceptableRelativeError", options.acceptableRelativeError}}},
                {"ethics", json{{"voluntaryOnly", true},
                                  {"auditableTasks", true},
                                                                    {"allowParticipantRoutingStorage", options.allowParticipantRoutingStorage},
                                                                    {"allowUnmatchedVolunteerCompute", options.ignoreMatchingForComputeNodes},
                                                                    {"participantStorageMeaning", options.allowParticipantRoutingStorage ? "lookup of volunteer capability signatures" : "no routing storage; only generic compute eligibility"},
                                  {"forbidPersistentHumanStorage", true},
                                  {"allowEphemeralMnemonicHints", options.allowMnemonicEncoding},
                                  {"forbidSensitivePayloads", true},
                                  {"revocableParticipation", true}}},
                {"estimatedSavings", json{{"baselineOps", baselineOps},
                                           {"guidedOps", guidedOps},
                                           {"relativeReduction", std::max(0.0, 1.0 - guidedOps / std::max(1.0, baselineOps))}}},
                {"thoughtModel", thoughtModel}};
}

inline json buildCollectiveConsciousComputePlan(const json &sessionState, CollectiveComputeOptions options = {}) {
    return buildCollectiveConsciousComputePlan(sessionState, json::object(), options);
}
...
inline json buildBrainProfile(const json &sessionState, BrainProfileOptions options = {}) {
    ...
    structural["consciousComputePlan"] = buildConsciousComputePlan(structural);
    structural["collectiveComputePlan"] = buildCollectiveConsciousComputePlan(structural);
...
inline std::string buildCognitivePromptContext(const json &cognitiveState, std::size_t maxCharsPerLine = 160) {
    ...
    if (cognitiveState.contains("corticalSystems") && cognitiveState["corticalSystems"].is_object()) {
        ...
        if (cognitiveState.contains("consciousComputePlan") && cognitiveState["consciousComputePlan"].is_object()) {
            const auto estimated = cognitiveState["consciousComputePlan"].value("estimatedSavings", json::object());
            std::ostringstream oss;
            oss << "world_conscious_compute|savings: branches="
                << std::fixed << std::setprecision(2) << estimated.value("branchReduction", 0.0)
                << ", tokens=" << std::fixed << std::setprecision(2) << estimated.value("tokenReduction", 0.0);
            lines.push_back(truncateText(oss.str(), maxCharsPerLine));
        }
        if (cognitiveState.contains("collectiveComputePlan") && cognitiveState["collectiveComputePlan"].is_object()) {
            const auto mesh = cognitiveState["collectiveComputePlan"].value("collectiveMesh", json::object());
            const auto estimated = cognitiveState["collectiveComputePlan"].value("estimatedSavings", json::object());
            std::ostringstream oss;
            oss << "world_collective_compute|mesh: participants=" << mesh.value("participantCount", 0)
                << ", shards=" << mesh.value("shardCount", 0)
                << ", reduction=" << std::fixed << std::setprecision(2) << estimated.value("relativeReduction", 0.0);
            lines.push_back(truncateText(oss.str(), maxCharsPerLine));
        }
    }
    return joinStrings(lines, "\n");
