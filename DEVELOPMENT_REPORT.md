...
- `/vision/analyze` now emits a JPEA image-world-model concept vector and persists it into the world model. It no longer invokes the legacy `VisionPipeline` YOLO/CNN route.
...
- `jpea_v2_image_world_model.cpp`, `rdk_x5_bpu.cpp`, and `module_overrides/gnn_ga_learner_advanced.cpp` pass local C++ syntax validation.
...
- No Horizon `.bin` or `.hbm` I-JEPA/JPEA model is present. The current JPEA implementation reports its fallback backend, so it must not be represented as BPU execution. A compatible quantized Horizon model and its exact input layout are required before wiring this route to `rdk_x5_bpu::execute`.
...
3. Convert and deploy a supported JPEA/I-JEPA model to Horizon format, then provide its model path, tensor layout, quantization, and preprocessing contract.
