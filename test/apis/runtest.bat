...
if exist "%LOG_DIR%\apitest.log" del /f /q "%LOG_DIR%\apitest.log" >nul 2>&1
if exist "%LOG_DIR%\apitest_report.json" del /f /q "%LOG_DIR%\apitest_report.json" >nul 2>&1
if exist "%LOG_DIR%\phoenix_main.log" del /f /q "%LOG_DIR%\phoenix_main.log" >nul 2>&1
...
"%VENV_PY%" -m pip install --upgrade pip > "%LOG_DIR%\pip_upgrade.log" 2>&1
if errorlevel 1 (
    echo [WARN] pip upgrade failed, continue. See logs\pip_upgrade.log
)

echo [INFO] Installing requirements
"%VENV_PY%" -m pip install -r "%SCRIPT_DIR%requirements.txt" > "%LOG_DIR%\pip_install.log" 2>&1
if errorlevel 1 (
    echo [ERROR] Failed to install requirements. See logs\pip_install.log
    popd
    exit /b 5
)
...
start "phoenix_main" /B "%PHOENIX_EXE%" %PHOENIX_ARGS% > "%LOG_DIR%\phoenix_main.log" 2>&1
if errorlevel 1 (
...
:ready_ok
if "%READY%"=="0" (
    echo [ERROR] phoenix_main frontend not ready in time. See "%LOG_DIR%\phoenix_main.log"
    popd
    exit /b 8
)

echo [INFO] Running API tests against: %BASE_URL%
"%VENV_PY%" "%SCRIPT_DIR%main.py" --base-url "%BASE_URL%" --log-file "%LOG_DIR%\apitest.log" --report-file "%LOG_DIR%\apitest_report.json"
set "EXITCODE=%ERRORLEVEL%"

if "%EXITCODE%"=="0" (
    echo [INFO] API tests passed
) else (
    echo [ERROR] API tests failed with code %EXITCODE%
)

echo [INFO] Logs: "%LOG_DIR%\apitest.log"
echo [INFO] Report: "%LOG_DIR%\apitest_report.json"
