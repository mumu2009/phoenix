...
void testConsciousComputePlanBuildsHumanInLoopStages() {
    json state = {
        {"sessionId", "brain-compute-1"},
        {"sceneState", {{"summary", "Observed a mechanic comparing two noisy pumps while looking for the failing unit."},
                         {"objectSlots", json::array({json{{"label", "left pump"}}, json{{"label", "right pump"}}})},
                         {"tags", json::array({"pump", "noise", "failure"})}}},
        {"episode", {{"summary", "One pump likely has abnormal vibration and should be isolated."}}},
        {"recentEvidence", json::array({json{{"modality", "vision"}, {"graphSummary", "two pumps with different vibration patterns"}},
                                         json{{"modality", "speech"}, {"text", "the left pump sounds rougher than the right one"}}})}
    };

    auto plan = world_model::buildConsciousComputePlan(state);

    requireTrue(plan.is_object(), "conscious compute plan should be an object");
    requireTrue(plan.value("mode", std::string()) == "human-in-the-loop-conscious-compute",
                "conscious compute plan should report the correct mode");
    requireTrue(plan.contains("humanStages") && plan["humanStages"].is_array() && !plan["humanStages"].empty(),
                "conscious compute plan should contain human stages");
    requireTrue(plan.contains("machinePrecompute") && plan["machinePrecompute"].is_array() && !plan["machinePrecompute"].empty(),
                "conscious compute plan should contain machine preprocessing steps");
    requireTrue(plan.contains("estimatedSavings") && plan["estimatedSavings"].is_object(),
                "conscious compute plan should estimate savings");
}

void testCollectiveComputePlanBuildsMatrixShardProtocol() {
    json state = {
        {"sessionId", "brain-collective-1"},
        {"sceneState", {{"summary", "Observed a clustered approximation task that should be solved through many small semantic votes."},
                         {"objectSlots", json::array({json{{"label", "left matrix"}}, json{{"label", "right matrix"}}, json{{"label", "semantic shards"}}})},
                         {"tags", json::array({"matrix", "approximation", "semantic"})}}},
        {"episode", {{"summary", "The system wants to approximate blockwise matrix multiplication with sparse human semantic judgments."}}},
        {"recentEvidence", json::array({json{{"modality", "vision"}, {"graphSummary", "matrix blocks grouped into semantic clusters"}},
                                         json{{"modality", "speech"}, {"text", "use approximate block voting with acceptable error"}}})}
    };

    json computeTask = {
        {"operation", "matrix_multiply"},
        {"inputA", "row-cluster-alpha"},
        {"inputB", "col-cluster-beta"},
        {"desiredOutput", "product-cluster-gamma"},
        {"leftShape", json::array({64, 64})},
        {"rightShape", json::array({64, 64})},
        {"blockRows", 16},
        {"blockCols", 16}
    };

    world_model::CollectiveComputeOptions options;
    options.participantCount = 24;
    options.shardCount = 6;
    options.redundancyFactor = 2;
    options.allowMnemonicEncoding = true;

    auto plan = world_model::buildCollectiveConsciousComputePlan(state, computeTask, options);

    requireTrue(plan.is_object(), "collective compute plan should be an object");
    requireTrue(plan.value("mode", std::string()) == "collective-semantic-compute",
                "collective compute plan should report the correct mode");
    requireTrue(plan.value("operation", std::string()) == "matrix_multiply",
                "collective compute plan should preserve the operation type");
    requireTrue(plan.contains("collectiveMesh") && plan["collectiveMesh"].is_object(),
                "collective compute plan should contain a collective mesh");
    requireTrue(plan["collectiveMesh"].contains("humanShards") && plan["collectiveMesh"]["humanShards"].is_array() && !plan["collectiveMesh"]["humanShards"].empty(),
                "collective compute plan should emit shard tasks");
    requireTrue(plan.contains("routingDirectory") && plan["routingDirectory"].is_object(),
                "collective compute plan should contain a routing directory");
    requireTrue(plan["routingDirectory"].value("storageMode", std::string()) == "participant-signature-lookup",
                "collective compute plan should store participant signatures rather than human memory content");
    requireTrue(plan["routingDirectory"].contains("routeQueries") && plan["routingDirectory"]["routeQueries"].is_array() && !plan["routingDirectory"]["routeQueries"].empty(),
                "collective compute plan should emit route queries");
    requireTrue(plan.contains("ethics") && plan["ethics"].is_object(),
                "collective compute plan should expose ethics constraints");
    requireTrue(plan["ethics"].value("allowParticipantRoutingStorage", false),
                "collective compute plan should explicitly allow participant routing storage");
    requireTrue(plan["ethics"].value("forbidPersistentHumanStorage", false),
                "collective compute plan should forbid persistent human storage");
}

void testCollectiveComputePlanCanIgnoreMatchingForGenericComputeNodes() {
    json state = {
        {"sessionId", "brain-collective-2"},
        {"sceneState", {{"summary", "Observed a volunteer mesh where routing signatures are optional for generic compute work."},
                         {"objectSlots", json::array({json{{"label", "volunteer mesh"}}, json{{"label", "routing table"}}})},
                         {"tags", json::array({"collective", "compute", "routing"})}}},
        {"episode", {{"summary", "Volunteers should still compute even when they are not stored as routing signatures."}}},
        {"recentEvidence", json::array({json{{"modality", "vision"}, {"graphSummary", "volunteers organized as generic compute nodes"}},
                                         json{{"modality", "speech"}, {"text", "matching matters only when selecting routing storage nodes"}}})}
    };

    json computeTask = {
        {"operation", "semantic_approximation"},
        {"inputA", "alpha"},
        {"inputB", "beta"},
        {"desiredOutput", "gamma"}
    };
    ...
    world_model::CollectiveComputeOptions options;
    options.participantCount = 12;
    options.shardCount = 3;
    options.redundancyFactor = 2;
    options.allowMnemonicEncoding = false;
    options.ignoreMatchingForComputeNodes = true;
    options.allowParticipantRoutingStorage = false;

    auto plan = world_model::buildCollectiveConsciousComputePlan(state, computeTask, options);

    requireTrue(plan.is_object(), "collective compute plan should be an object");
    requireTrue(plan["collectiveMesh"].contains("genericComputePool"),
                "collective compute plan should expose a generic compute pool");
    requireTrue(plan["collectiveMesh"]["genericComputePool"].value("matchingImportance", std::string()) == "low",
                "generic compute pool should mark matching importance as low");
    requireTrue(plan["routingDirectory"].value("matchingPriority", std::string()) == "routing-storage-only",
                "routing directory should restrict signature matching to routing storage mode");
    requireTrue(!plan["ethics"].value("allowParticipantRoutingStorage", true),
                "routing storage should be disabled when not requested");
    requireTrue(plan["ethics"].value("allowUnmatchedVolunteerCompute", false),
                "unmatched volunteers should remain eligible as compute nodes");
}
...
int main() {
    try {
        testBuildBrainProfilesProvidesFunctionalAndStructuralVersions();
        testFunctionalBrainRuntimeIncludesControlAndActionLoops();
        testCognitiveStateIncludesReasoningAgenda();
        testBrainProfileIncludesResponsePlan();
        testResponsePlanBuildsGroundMobilityPlanForRover();
        testBuildCognitiveStateIncludesWorkingMemoryGoalsAndRegions();
        testWorldModelStorePersistsPredictiveStateAndCalibration();
        testWorldModelStoreDetectsPredictionMismatchAndFeedsPromptContext();
        testSimulateVirtualSceneCreatesMultipleAgentsAndTimeline();
        testVirtualSceneTrainingSampleLimitAndTruncation();
        testStructuralBrainModePropagatesIntoSimulation();
        testVirtualSceneBuildsEmbodied3DWorldAndEcologyFromVideo();
        testConsciousComputePlanBuildsHumanInLoopStages();
        testCollectiveComputePlanBuildsMatrixShardProtocol();
        testCollectiveComputePlanCanIgnoreMatchingForGenericComputeNodes();
        std::cout << "world_model_scene_brain_tests: ok" << std::endl;
        return 0;
    } catch (const std::exception &ex) {
        std::cerr << "world_model_scene_brain_tests: failed: " << ex.what() << std::endl;
        return 1;
    }
}